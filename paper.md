...
Taken to its conclusion, this authoring model also answers maintenance drift. When Sparse is the substrate rather than a parallel version, whatever you write is Sparse the moment you write it — and nothing can appear in Luxe that Sparse doesn't already carry. There is no second surface to keep in sync, because there is no second surface.
...
1. **Content equivalence verification.** How should tooling verify that profiles agree on title, summary, state, and critical actions? The deeper answer is architectural rather than forensic: **equivalence by construction**. When S—ML is used as a *system* — the Sparse representation as the single substrate that Medium and Luxe enhance — drift is structurally impossible: the moment words are written they are Sparse-available, and anything absent from S cannot exist in a higher profile at all. Under that authoring model there is nothing to verify. Verification tooling is then needed only for the remaining cases — retrofits, separately-authored M and L, and stacks that generate content inside client-side JavaScript (virtual-DOM frameworks are the canonical leak) — where a hierarchical correspondence check (matching headings, sections, and actions across representations) is the likely direction.
...
3. **Advertising.** How do ad-supported publishers participate? Sparse's budgets exclude third-party ad tech by construction, but advertising itself is not banned: a first-party, inline, lightweight ad fits within budget. A concrete direction: a **Sparse ad unit** that is deliberately typographic — a single line or short styled block, character and typography doing the work that imagery does elsewhere, closer to a classified or a well-set print notice than to programmatic display. Constraint breeds craft: such a format would reward copywriting over surveillance. Whether a standardized unit emerges, and whether publishers accept profile-dependent monetization, is open — and probably decisive for news adoption.
4. **Declarative profile syntax.** Should profiles get first-class CSS/HTML syntax, letting authors express delivery weight in the same idiom responsive design uses for screens — for example a media query like `@media (profile: sparse)`, aligned with the existing `prefers-reduced-data` proposal, or standardized attributes for profile-gated resources? A note on mechanism: a runtime media query can gate *rendering* but not *transfer* — by the time CSS is evaluated, the bytes are already paid for. The workable version is **profile syntax as a build-time contract**: author one stylesheet with profile-scoped blocks (`@profile s { … }`), and let the build tool or server strip it to the requested profile at delivery time. That keeps authors in one file, makes per-profile CSS weight statically analyzable — an auditor can measure the `s` block alone — and formalizes what critical-CSS extraction already does ad hoc.
